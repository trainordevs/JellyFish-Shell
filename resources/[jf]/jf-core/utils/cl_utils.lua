...
--[[
    amt - amount of money
    gaining (true/false) - is the player gaining money?
]]
function Util.FormatMoneyString(amt, gaining)
	if gaining then
		return '(~g~$' .. amt .. '~w~)'
	else
		return '(~r~$' .. amt .. '~w~)'
...
function Util.PrependTable(t, v)
	if not t then print('jf-core (clients/cl_utils.lua) [Util.PrependTable] - You need to provide a table in argument position 1.') return end
	if not v then print('jf-core (clients/cl_utils.lua) [Util.PrependTable] - You need to provide a value in argument position 2.') return end
	if #t > 0 and not t[1] then print('jf-core (clients/cl_utils.lua) [Util.PrependTable] - Table provided is not using numbers for keys.') return end
